...
product.get = function(req, res, next) {
    ...
    if (req.params.id) {
        log.info({ message: 'Retrieving product with ID : ' + req.params.id });
...
product.create = function(req, res, next) {
    log.info({ req: req.body }, { message: 'POST request to /product received' });
    // get all products from model
...
product.update = function(req, res, next) {
    log.info({ req: req.body }, { message: 'PUT request to /product received' });
